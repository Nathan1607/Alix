...
const Commune = () => {
          ...
          itemsToRender.push(
            ...articles.map(article => (
              <CardItem
                ...
                imageUrl={article.img_1 ?? 'https://via.placeholder.com/300x200?text=Image+non+disponible'}
                tag="Article"
                tagColor="#8B0000"
        ...
        if (activeTab === 'Voir tout' || activeTab === 'Événements') {
          itemsToRender.push(
            ...events.map(event => (
              <CardItem
                key={`event-${event.id}`}
                imageUrl={event.img_1 ?? 'https://via.placeholder.com/300x200?text=Image+non+disponible'}
                tag="Événement"
                tagColor="#004B8D"
                title={event.title_1}
          ...
          itemsToRender.push(
            ...workshops.map(atelier => (
              <CardItem
                ...
                imageUrl={atelier.img_1 ?? 'https://via.placeholder.com/300x200?text=Image+non+disponible'}
                tag="Atelier"
                tagColor="#007B55"
  ...
  style={{
    ...
    padding: "1rem",
    gap: "1.5rem",
    scrollSnapType: "x mandatory", // Bonus : effet snap sur les cartes
  }}
>
  {itemsToRender}
</div>
